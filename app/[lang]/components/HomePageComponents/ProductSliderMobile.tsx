...
export default function ProductSliderMobile(props: any) {
    ...
    const SliderProduct = ({ sliders, lang, origin }: any) => {
        ...
        const renderSlides = () =>
            sliders.map((data: any, i: number) => {
                ...
                const expressImage = isArabic ? `/icons/express_logo/Express_ar_48.webp` : `/icons/express_logo/Express_en_48.webp`
                ...
                return (
                    <SwiperSlide key={data?.id} className='pb-8'>
                        <div className='proBoxMob relative'>
                            {customBadge ?
                            ...
                            {useCodeText ?
                                <div className={`text-[0.60rem] font-semibold bg-${useCodeTextColor}/15 py-1 px-1.5 rounded-md mt-1 w-fit`} style={{backgroundColor:`${useCodeTextColor}15`}}>
                            ...
                            <div className='overflow-hidden'>
                                <div className='absolute ltr:right-1 rtl:left-1 top-1 z-10'>
                            ...
                            <Link href={productLink} className=''>
                                ...
                                {ProExtraData[data?.id]?.expressdeliveryData ?
                                    <Image
                                        ...
                                        width={0}
                                        loading='lazy'
                                        className='rounded-md my-2 w-full h-auto'
                                        sizes='(max-width: 640px) 100vw, (max-width: 768px) 100vw, (max-width: 1024px) 100vw, 100vw'
                                    />
                                ...
                                {shortDescriptionAvailable ?
                                    <div className='bg-gray/10 p-2 rounded-md mt-2'>
                                        <h3 className='font-normal text-[0.45rem]'>{installmentPlans}</h3>
                                        {/* <div className='flex items-center gap-1 justify-between overflow-hidden'> */}
                                        <div className="w-full overflow-hidden">
                                            {/* {dataInstallment?.map((data, i) => ( */}
                                            <Image
        ...
        return (
            <Swiper
                ...
                className={`mySwiper${randomTwoDigit}`}
            >
                {renderSlides()}
            </Swiper>
        );
    ...
    return (
        <>
            {props?.headingRequired ?
                <div className={`align__center mb-2 `}>
                    <h2 className="headingHomeMobile">{props?.title}</h2>
                    <div><button className="text-sm underline text-primary font-semibold"
                        onClick={() => {
                            router.push(`${origin}/${isArabic ? 'ar' : 'en'}/${props?.buttonLink}`)
                        }}
                    >{props?.showAllTitle}</button></div>
                </div>
                : null}
            <SliderProduct
