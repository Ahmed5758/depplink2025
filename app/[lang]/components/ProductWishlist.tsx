...
export default function ProductWishlist(props: any) {
            ...
            if (type) {
                post('removewishlist', data).then((responseJson: any) => {
                    if (responseJson?.success) {
                        ...
                        topMessageAlartDanger(props?.dict?.products?.wishlistRemovedText)
                ...
                post('addwishlist', data).then((responseJson: any) => {
                    if (responseJson?.success) {
                        // testing[id].wishlist = !type;
                        // setProWishlistData({ ...testing })
                        topMessageAlartSuccess(props?.dict?.products?.wishlistAddedText)
                        if (localStorage.getItem("wishlistCount")) {
                            var wishlistlength: any = localStorage.getItem('wishlistCount');
                            wishlistlength = parseInt(wishlistlength) + 1;
                            localStorage.setItem('wishlistCount', wishlistlength);
            ...
            if (type) {
                post('removecompare', data).then((responseJson: any) => {
                    if (responseJson?.success) {
                        ...
                        topMessageAlartDanger(props.dict?.products?.compareRemovedText)
                ...
                post('addcompare', data).then((responseJson: any) => {
                    if (responseJson?.success) {
                        // testing[id].compare = !type;
                        // setProComparetData({ ...testing })
                        topMessageAlartSuccess(props.dict?.products?.compareAddedText)
                        if (localStorage.getItem("compareCount")) {
                            var comparelength: any = localStorage.getItem('compareCount');
                            comparelength = parseInt(comparelength) + 1;
                            localStorage.setItem('compareCount', comparelength);
                        ...
                        topMessageAlartDanger(props.dict?.products?.compareAlreadyText)
                    }
                })
            }
        } else {
    ...
    const dataInstallment = [
        {
            image: "/images/pro_baseeta.webp",
            alt: isArabic ? "" : "baseeta",
            title: isArabic ? "" : "Baseeta",
        },
        {
            image: "/images/pro_tabby.webp",
            alt: isArabic ? "" : "tabby",
            title: isArabic ? "" : "Tabby",
        },
        {
            image: "/images/pro_tamara.webp",
            alt: isArabic ? "" : "tamara",
            title: isArabic ? "" : "Tamara",
        },
        {
            image: "/images/misspay.webp",
            alt: isArabic ? "" : "mispay",
            title: isArabic ? "" : "Mispay",
        },
        {
            image: "/images/pro_madfu.webp",
            alt: isArabic ? "" : "madfu",
            title: isArabic ? "" : "Madfu",
        },
    ]
    return (
        <>
            <div className={`grid grid-cols-${props?.grid ? props?.grid : 6} items-start justify-center gap-3`}>
                {props?.products?.map((data: any, i: number) => {
                    const productLink = `${origin}/${isArabic}/product/${data?.slug}`
                    ...
                    const proIimageAlt = isArabic == 'ar' ? data?.name_arabic : data?.name
                    const productBrand = isArabic == 'ar' ? data?.brand?.name_arabic : data?.brand?.name
                    const proTitle = isArabic == 'ar' ? data?.name_arabic : data?.name
                    const pormotionType = isArabic == 'ar' ? 'عرض' : 'Offer';
                    const installmentPlans = isArabic == 'ar' ? 'خطط التقسيط' : 'Split it your way buy now pay later!';
                    // const useCodeText = isArabic == 'ar' ? `استخدم كود الخصم ${`RMD12`}` : `Use code ${`RMD12`}`
                    // const customBadge = isArabic == 'ar' ? "أكثر مبيعا" : "Selling Out Fast"
                    const useCodeText = isArabic == 'ar' ? data?.badge_right_arabic ?? '' : data?.badge_right ?? '';
                    const useCodeTextColor = data?.badge_right_color ? data?.badge_right_color : 'greenDark';
                    const customBadge = isArabic == 'ar' ? data?.badge_left_arabic ?? '' : data?.badge_left ?? '';
                    const customBadgeColor = data?.badge_left_color ? data?.badge_left_color : 'red';
                    const pormotionTitle = isArabic == 'ar' ? data?.pormotion_arabic ?? '' : data?.pormotion ?? '';
                    ...
                    const productSalePrice = data?.promotional_price > 0 ? (getFormattedPrice(Math.round((getDiscountedPrice() - data?.promotional_price))).toLocaleString()) : (getFormattedPrice(getDiscountedPrice()))
                    ...
                    const percentageAmount = Math.round(((data?.price - flashCalc) * 100) / data?.price)
                    ...
                    const FreeGiftData: React.FC<FreeGiftProps> = ({ freeGiftType, freeGiftAllowed, isSelectGift, freeGiftList }) => {
                        return (
                            <div className="text-[0.65rem] font-semibold">
                                {freeGiftType == "0" ?
                                    <span className="text-specialDarkYellow flex items-center gap-1"><Image src="/icons/gift.svg" height={16} width={16} alt="giftIcon" title='Gift Icon' className='' />{isArabic == 'ar' ? `هدية مجانية ${freeGiftAllowed}` : `${freeGiftAllowed} Free Gift ${freeGiftAllowed >= 2 ? '(s)' : ''}`}</span>
                                    :
                                    freeGiftType == "1" ?
                                        <span className="text-red flex items-center gap-1"><Image src="/icons/gift.svg" height={16} width={16} alt="giftIcon" title='Gift Icon' className='' />{isArabic == 'ar' ? `هدية مجانية ${freeGiftAllowed} اختر` : `Select ${freeGiftAllowed} Free Gift`}</span>
                                        // :
                                        // freeGiftType == "2" ?
                        ...
                        return (
                            <div className="text-xs font-semibold">
                                {data?.custom_badge_en || data?.custom_badge_ar ?
                                    <>
                                        <span className="text-greenDark flex items-center gap-1">{isArabic == "ar" ? "وفر " : "Save"}{" "}{isArabic == "ar" ? data?.custom_badge_ar : data?.custom_badge_en}{" "}{currencySymbol}</span>
                                    </>
                                    :
                                    <>
                                        {isSale ? (
                                             <span className="text-greenDark flex items-center gap-1">{isArabic == "ar" ? "وفر " : "Save"}{" "}{parseInt(regPriceStr?.replace(/,/g, ''), 10) - parseInt(salePriceStr?.replace(/,/g, ''), 10)}{" "}{currencySymbol}</span>
                                        ) : (
                                            <span className="text-red">{discountPercentage}% {isArabic == "ar" ? "خصم" : "OFF"}</span>
                                        )}
                                    </>
                                }
                            </div>
                        ...
                        props?.devicetype == true ?
                            <div className='proBox relative'>
                                <div className="flex justify-between">
                                    <div className='space-y-2'>
                                        {customBadge ?
                                            <div className="text-[0.60rem] font-semibold bg-greenDark/15 py-1 px-1.5 rounded-md w-fit">
                                                {/* <span className="text-greenDark animationImp">{customBadge}</span> */}
                                                <span className={`text-${customBadgeColor} animationImp`}>{customBadge}</span>
                                            </div>
                                            : null}
                                        {useCodeText ?
                                            <div className="text-[0.60rem] font-semibold bg-red/15 py-1 px-1.5 rounded-md w-fit">
                                                {/* <span className="text-red">{useCodeText}</span> */}
                                                <span className={`text-${useCodeTextColor}`}>{useCodeText}</span>
                                            </div>
                                            : null}
                                    </div>
                                    ...
                                    <div className='align__center bg-gray/10 p-2 rounded-md'>
                                        <div className=''>
                                            ...
                                            <div className='flex items-center'>
                                                ...
                                                <h4 className='text-sm'><DiscountLabel isSale={saleType} discountPercentage={percentageAmount} regPrice={getFormattedPrice(getOriginalPrice())} salePrice={productSalePrice} /></h4>
                                            </div>
                            ...
                            <div className='proBox relative'>
                                <div className='align__center'>
                                    {customBadge ?
                                        <div className="text-[0.60rem] font-semibold bg-greenDark/15 py-1 px-1.5 rounded-md">
                                            {/* <span className="text-greenDark animationImp">{customBadge}</span> */}
                                            <span className="text-greenDark animationImp">{customBadge}</span>
                                        </div>
                                        : null}
