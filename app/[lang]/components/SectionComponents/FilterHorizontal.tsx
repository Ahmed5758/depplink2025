...
export default function FilterHorizontal(props: FilterProps) {
  ...
  const dropdownIconRight = (
    <svg
      ...
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      className={`fill-current transform group-hover:translate-x-1 transition duration-150 ease-in-out ${isArabic ? "rotate-180" : ""
        }`}
    >
      <path
        clipRule="evenodd"
  ...
  return (
    <div
      className={`filter_wrapper bg-white rounded-[1.438rem] md:p-[1.6875rem] md:pb-0 p-4 shrink-0 overflow-hidden shadow-md`}
    >
      {/* Filter Header */}
      ...
      <div className="w-full flex md:flex-row flex-col md:items-start items-center gap-2">
        ...
        <div className="filter_left md:w-1/2 w-full">
          {leftTags.map((tagdata, t) => (
            <div
              key={t}
              className="flex md:flex-row flex-col md:items-center lg:gap-[1.5rem] gap-3 w-full overflow-hidden mb-[1.188rem]"
            >
              <p className="2xl:text-sm text-xs text-[#252B42] font-bold flex items-center gap-[.625rem] w-full 2xl:w-[6rem] md:max-w-28 relative">
                ...
                {tagdata.childs.filter(
                  ...
                  <span className="rounded-[.6375rem] w-4 h-4 bg-primary text-white shrink-0 inline-flex items-center justify-center text-[.6375rem] font-medium md:absolute ltr:md:-right-3 rtl:md:-left-3 md:top-1/2 md:-translate-y-1/2">
                    {
                      tagdata.childs.filter(
                        (child) => props.selectedtags[child.name]
                      ).length
              ...
              <div className="relative w-full overflow-hidden">
                {isArabic ? (
                  <div
                    ref={nextRef}
                    className="absolute left-0 top-0 bottom-0 z-10 cursor-pointer text-primary w-9 flex items-center justify-end pointer-events-auto"
                    style={{
                      background:
                        "linear-gradient(90deg, #FFF 40.55%, rgba(246, 247, 247, 0.05) 100%)",
                    }}
                  >
                    {dropdownIconRight}
                  </div>
                ...
                <Swiper
                  ...
                  breakpoints={{
                    ...
                    1280: { slidesPerView: "auto", spaceBetween: 12 },
                  }}
                  ...
                  className={`!w-full ${isArabic ? "!pl-5" : "!pr-5"}`}
                >
                  {tagdata.childs.map((tagchild, i) => {
                    const isSelected =
                      props.selectedtags[tagchild.name] === true;
                    return (
                      <SwiperSlide
                        key={i}
                        className="!w-auto min-w-fit max-w-[80%] md:max-w-[60%]"
                      >
                        <button
                          onClick={() => props.onChangetags(tagchild)}
                          className={`relative w-full 2xl:h-10 h-8 rounded-full cursor-pointer outline-none flex items-center justify-center px-4 ${isSelected
                            ? "border-2 border-primary bg-white"
                            : "border-[0.2px] border-[#121212] bg-[#F3F3F3]"
                            }`}
                          aria-label={
        ...
        {/* <div className="w-px 2xl:h-[17.5rem] h-[9rem] bg-[#D9D9D9]/20 rounded-[.625rem] 2xl:mx-6 mx-[13.5px] 2xl:block hidden"></div> */}

        {/* Filter Right */}
        <div className="filter_right md:w-1/2 w-full">
          {/* Brand Section */}
          {props.brands?.length > 0 && (
            <div className="flex md:flex-row flex-col md:items-center lg:gap-[1.5rem] gap-3 w-full overflow-hidden mb-[1.188rem]">
              ...
              <div className="relative w-full overflow-hidden">
                ...
                >
                  {props.brands.map((brand, i) => {
                    const isSelected =
                      props.selectedbrands[brand.name] === true;
                    return (
                      <SwiperSlide
                        key={brand.id}
                        className="!w-auto min-w-fit max-w-[80%] md:max-w-[60%]"
                      >
                        <button
                          ...
                          className={`relative w-full 2xl:h-10 h-8 px-4 py-2.5 rounded-full cursor-pointer outline-none ${isSelected
                            ? "border-2 border-primary bg-white"
                            : "border-[0.2px] border-[#121212] bg-[#F3F3F3]"
                            }`}
