...
import './HomePageComponents/styles.css';  // Ensure to import custom styles for the scrollbar
...
export default function ProductsSliderSmallThumb(props: any) {
    ...
    const sliderHeading = isArabic ? "props?.heading_ar" : "props?.heading_en"
    const sliderHeadingRequired = props?.headingRequired ? true : false
    const randomTwoDigit = Math.floor(Math.random() * 90) + 10;

    const uniqueId = Math.random().toString(36).substring(7); // Generate a random ID
    const SliderProduct = ({ sliders, lang, origin }: any) => {
        if (!sliders?.length) return null;

        const renderSlides = () =>
            sliders?.map((data: any, i: number) => {
                const productLink = `${origin}/${isArabic ? 'ar' : 'en'}/product/${data?.slug}`
                    ...
                    <SwiperSlide key={data?.id}>
                        <div className='proBox relative'>
                            ...
                            <Link href={productLink} className=''>
                                ...
                                <h2 className='line-clamp-2 min-h-8'><span className='font-bold after:content-["•"]'>{productBrand}{' '}</span>{' '}{proTitle}</h2>
        ...
        return (
            <Swiper
                spaceBetween={10}
                slidesPerView={sliderHeadingRequired ? 4 : 6}
                autoHeight={true}
                loop={true}
                breakpoints={{
                    320: {
                    ...
                    768: {
                        slidesPerView: sliderHeadingRequired ? 3.8 : 2.8,
                        spaceBetween: 10,
                    },
                    1024: {
                        slidesPerView: sliderHeadingRequired ? 4.8 : 3.8,
                        spaceBetween: 10,
                    },
                    1650: {
                        slidesPerView: sliderHeadingRequired ? 5 : 4,
                        spaceBetween: 10,
                    },
                    1920: {
                        slidesPerView: sliderHeadingRequired ? 6 : 5,
                        spaceBetween: 10,
                    },
                }}
                ...
                freeMode={true}
                pagination={false}
                modules={[Scrollbar, Mousewheel, Grid, FreeMode]}
                navigation={{ nextEl: `.arrow-left-${props?.sec}`, prevEl: `.arrow-right-${props?.sec}` }}
                className={`mySwiper${randomTwoDigit} !pb-8`}
            >
                {renderSlides()}
            </Swiper>
        );
    ...
    return (
        <>
            ...
            {/* <div className="flex items-center justify-between gap-1 absolute z-10 top-[45%] w-full">
                <button
                    aria-label="Previous"
                    className={`${props.lang === 'ar' ? 'rotate-180' : ''} cursor-pointer fill-dark arrow-right-${props?.sec} arrow p-4 bg-dark/40 rounded-full`}
                >
                    <svg height="16" viewBox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg" className="fill-current transform transition duration-150 ease-in-out rotate-90">
                        <path clipRule="evenodd" d="m2.58579 7.58579c.78104-.78105 2.04738-.78105 2.82842 0l6.58579 6.58581 6.5858-6.58581c.781-.78105 2.0474-.78105 2.8284 0 .7811.78104.7811 2.04738 0 2.82841l-8 8c-.781.7811-2.0474.7811-2.8284 0l-8.00001-8c-.78105-.78103-.78105-2.04737 0-2.82841z" fillRule="evenodd"></path>
                    </svg>
                </button>
                <button
                    aria-label="Next"
                    className={`${props.lang === 'ar' ? 'rotate-180' : ''} cursor-pointer fill-dark arrow-left-${props?.sec} arrow p-4 bg-dark/40 rounded-full`}
                >
                    <svg height="16" viewBox="0 0 24 24" width="16" xmlns="http://www.w3.org/2000/svg" className="fill-current transform transition duration-150 ease-in-out -rotate-90">
                        <path clipRule="evenodd" d="m2.58579 7.58579c.78104-.78105 2.04738-.78105 2.82842 0l6.58579 6.58581 6.5858-6.58581c.781-.78105 2.0474-.78105 2.8284 0 .7811.78104.7811 2.04738 0 2.82841l-8 8c-.781.7811-2.0474.7811-2.8284 0l-8.00001-8c-.78105-.78103-.78105-2.04737 0-2.82841z" fillRule="evenodd"></path>
                    </svg>
