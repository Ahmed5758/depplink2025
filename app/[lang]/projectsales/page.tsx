...
export default function ProjectSales({ params }: { params: { lang: string, data: any, devicetype: any } }) {
    ...
    const SubmitData = () => {
        ...
        post('mob-new-projectsale', data).then((responseJson: any) => {
            if (responseJson?.success === "true") {
                topMessageAlartSuccess(params.lang === 'ar' ? "تم إرسال استفسارك!" : "your inquiry has been sent!")
                setFirstName('')
                setLastName('')
                setEmail('')
                setPhoneNumber('')
    ...
    return (
        <>
            <MobileHeader type="Third" lang={params.lang} pageTitle={params.lang == 'ar' ? 'مبيعات المشاريع' : 'Project Sales'} />
            <div className="container py-16 md:py-4">
                <div className="md:my-6 w-full">
                    {/* <Image
                        ...
                        alt={params.lang == 'ar' ? 'مبيعات المشاريع' : 'projectsales'}
                        title={params.lang == 'ar' ? 'مبيعات المشاريع' : 'Project Sales'}
                        height={0} width={0} className="h-auto w-full rounded-md mb-5"
                    /> */}
                    <div className="w-full md:w-[400px] lg:w-[540px] mx-auto">
                        <h1 className="font-semibold text-sm md:text-base">{params.lang === 'ar' ? 'برجاء تعبئة الخانات التالية لطلب الخدمة' : 'Please fill out the following fields to request the service.'}</h1>
                        <div className="mt-2">
                            <div className="mb-1">
                                ...
                                <div className="pb-3 pt-2.5 px-3 bg-white rounded-md border flex items-center border-[#5D686F30] gap-x-2">
                                    <input className="focus-visible:outline-none w-1/2 text-xs font-normal" type="text" placeholder={params.lang === 'ar' ? 'الاسم الأول' : 'First Name'} value={firstName} onChange={(e) => {
                                        setFirstName(e.target.value)
                                    }}
                                    />
                                    <div className="border-r border-[#5D686F30] h-6"></div>
                                    <input className="focus-visible:outline-none w-1/2 text-xs font-normal" type="text" placeholder={params.lang === 'ar' ? 'اسم العائلة' : 'Last Name'} value={lastName} onChange={(e) => {
                            ...
                            <div className="mb-1">
                                <label className="text-xs font-normal">{params.lang === 'ar' ? 'اسم الشركة' : 'Company Name'}</label>
                                <div className="pb-3.5 pt-3 px-3 bg-white rounded-md border flex items-center border-[#5D686F30] gap-x-2">
                                    <input className="focus-visible:outline-none w-1/2 text-xs font-normal" placeholder={params.lang === 'ar' ? 'اسم الشركة' : 'Company Name'} type="text" value={compnanyName} onChange={(e) => {
                                        setCompnanyName(e.target.value)
                                    }} />
                                </div>
                            </div>
                            <div className="mb-1">
                                <label className="text-xs font-normal">{params.lang === 'ar' ? 'المدينة' : 'City'}</label>
                                <Select
                                    styles={{
                            ...
                            <div>
                                <label className="text-xs font-normal">{params.lang === 'ar' ? 'التفاصيل' : 'Details'}</label>
            ...
            <div className="fixed bottom-0 py-3 bg-white w-full border-t border-[#00000010] flex items-center justify-between container gap-2">
                ...
                >
                    {params.lang == 'ar' ? "تقديم الطلب" : "Submit Request"}
