...
export default function ProductWishlist(props: any) {
    ...
    const removeWishlistProduct = (id: number, alert: boolean = false) => {
        var data = {
        ...
        post('removewishlist', data).then((responseJson: any) => {
            if (responseJson?.success) {
                if (alert) {
                    topMessageAlartSuccess(props.dict?.wishlistRemovedText)
                    // topMessageAlartDanger(props.dict?.wishlistRemovedText)
                } else {
                    topMessageAlartSuccess(props.dict?.productCart)
                }
                props.refreshData()
    ...
    return (
        <>
            {ProData?.map((data: any, i: React.Key | null | undefined) => {
                ...
                return (
                    <>
                        <div className={`h-auto bg-white relative p-2 rounded-lg shadow-md w-full ${props?.className}`}>
                            {/* mb-1.5  */}
                            <Link
                                href={`${origin}/${props.lang}/product/${data.slug}`}
                                as={`${origin}/${props.lang}/product/${data.slug}`}
                                aria-label={isArabic ? data?.name_arabic : data?.name}
                                prefetch={true}
                                replace={true}
                                key={i}
                            >
                                {props?.devicetype === 'mobile' ? null
                                    ...
                                    ProExtraData[data?.id]?.expressdeliveryData ?
                                        ...
                                        moment(d2).diff(d1, 'days') <= data.newtype ?
                                            <div className='text-[#20831E] text-[0.55rem] md:text-xs absolute ltr:left-0 rtl:right-0 top-0 bg-[#20831E20] md:px-3.5 px-2 py-1 rtl:rounded-bl-lg rtl:rounded-tr-lg ltr:rounded-br-lg ltr:rounded-tl-lg z-20'>{isArabic ? 'جديد' : 'New'}</div>
                                            : data.best_seller == 1 ?
                                                <div className='text-[#20831E] text-[0.55rem] md:text-xs absolute ltr:left-0 rtl:right-0 top-0 bg-[#20831E20] md:px-3.5 px-2 py-1 rtl:rounded-bl-lg rtl:rounded-tr-lg ltr:rounded-br-lg ltr:rounded-tl-lg z-20'>{isArabic ? 'أكثر مبيعا' : 'Selling Out Fast'}</div>
                                                : data.top_selling == 2 ?
                                                    <div className='text-[#0B5ED8] text-[0.55rem] md:text-xs absolute ltr:left-0 rtl:right-0 top-0 bg-[#0B5ED820] md:px-3.5 px-2 py-1 rtl:rounded-bl-lg rtl:rounded-tr-lg ltr:rounded-br-lg ltr:rounded-tl-lgv'>{isArabic ? 'أكثر مبيعا' : 'Top Selling'}</div>
                                                    : data.low_in_stock == 3 ?
                                                        <div className='text-[#F0660C] text-[0.55rem] md:text-xs absolute ltr:left-0 rtl:right-0 top-0 bg-[#F0660C20] md:px-3.5 px-2 py-1 rtl:rounded-bl-lg rtl:rounded-tr-lg ltr:rounded-br-lg ltr:rounded-tl-lg z-20'>{isArabic ? 'كمية محدودة' : 'Low in Stock'}</div>
                                                        : null}
                                {data?.custom_badge_en || data?.custom_badge_ar ?
                                    <>
                                        <div className='text-[#EA4335] text-[0.55rem] md:text-xs absolute ltr:right-0 rtl:left-0 top-0 bg-[#EA433520] md:px-3.5 px-2 py-1 rtl:rounded-tl-lg rtl:rounded-br-lg ltr:rounded-bl-lg ltr:rounded-tr-lg z-20'>
                                            {isArabic ? data?.custom_badge_ar : data?.custom_badge_en}
                                        </div>
                                        ...
                                        {data?.sale_price ?
                                            <div className='text-[#EA4335] text-[0.55rem] md:text-xs absolute ltr:right-0 rtl:left-0 top-0 bg-[#EA433520] md:px-3.5 px-2 py-1 rtl:rounded-tl-lg rtl:rounded-br-lg ltr:rounded-bl-lg ltr:rounded-tr-lg z-20 flex gap-x-1 items-center'>
                                                {data?.savetype == 1 ?
                                                    ...
                                                    :
                                                    isArabic ? 'وفر ' + currencySymbol + (data.price - flashCalc).toLocaleString('EN-US') : 'Save ' + currencySymbol + (data.price - flashCalc).toLocaleString('EN-US')
                                                }
                                            </div>
                                            : null}
                                    </>
                                ...
                                <div className='relative'>
                                    <Image
                                        ...
                                        alt={isArabic ? data?.brand?.name_arabic + data?.name_arabic : data?.brand?.name + data?.name}
                                        title={isArabic ? data?.brand?.name_arabic + data?.name_arabic : data?.brand?.name + data?.name}
                                        height={260}
                                        width={260}
                                        loading='lazy'
                                        className='mx-auto'
                                ...
                                <h2 className='text-primary  font-semibold md:text-sm line-clamp-2 text-xs'>{isArabic ? data?.name_arabic : data?.name}</h2>
                                <div className='text-primary text-xs flex items-center gap-x-2'>
                                    ...
                                    {data?.brand?.brand_media_image ?
                                        <Image
                                            ...
                                            alt={ProExtraData[data?.id]?.badgeData?.badge_slider ? isArabic ? ProExtraData[data?.id]?.badgeData?.badge_slider?.title_arabic : ProExtraData[data?.id]?.badgeData?.badge_slider?.title : ''}
                                            title={ProExtraData[data?.id]?.badgeData?.badge_slider ? isArabic ? ProExtraData[data?.id]?.badgeData?.badge_slider?.title_arabic : ProExtraData[data?.id]?.badgeData?.badge_slider?.title : ''}
                                            height={30}
                                            width={80}
                                            loading='lazy'
                                        />
                                        :
                                        <p className="text-[#004B7A] text-sm">{isArabic ? data?.brand?.name_arabic : data?.brand?.name}</p>
                                    }
                                </div>
                                ...
                                {props?.devicetype === 'mobile' ?
                                    (ProExtraData[data?.id]?.expressdeliveryData ?
                                        <Image
                                            src={isArabic ? "/icons/express_logo/Express_ar_48.webp" : "/icons/express_logo/Express_en_48.webp"}
                                ...
                                {ProExtraData && Object.keys(ProExtraData).length && ProExtraData[data?.id]?.freegiftData ?
                                    <div className='flex items-end gap-x-2 mt-2'>
                                        ...
                                        <div className='text-xs font-regular'>
                                            {isArabic ? "يوجد هدية مع الـمنتج" : "With Free Gift"}
                                        </div>
                                    </div>
                                    :
                                    null
                                ...
                                {ProExtraData && Object.keys(ProExtraData).length && ProExtraData[data?.id]?.badgeData ?
                                    <Image
                                        ...
                                        alt={isArabic ? ProExtraData[data?.id]?.badgeData?.badge_slider?.title_arabic : ProExtraData[data?.id]?.badgeData?.badge_slider?.title}
                                        title={isArabic ? ProExtraData[data?.id]?.badgeData?.badge_slider?.title_arabic : ProExtraData[data?.id]?.badgeData?.badge_slider?.title}
                                        height={0}
                                        width={0}
                                        loading='lazy'
                            ...
                            <div className='align__center mt-3.5 gap-x-1.5'>
                                {data?.pre_order == 1 ?
                                    <>
                                        <button className='bg-primary focus-visible:outline-none text-white flex items-center justify-center px-1 py-2 gap-x-1 rounded-md md:h-10 h-8 w-full'
                                            aria-label={isArabic ? "اضـافـة الـي العـربــة" : "Add to Cart"}
                                            ...
                                            {buyNowLoading == data?.id ?
                                                ...
                                                <>
                                                    ...
                                                    <div className='text-xs  font-semibold max-md:hidden'>
                                                        {isArabic ? 'اضف الي العربة' : 'Pre Order'}</div>
                                                </>
                                            }
                                        </button>
                                    </>
                                    :
                                    <button className='bg-primary focus-visible:outline-none text-white flex items-center justify-center px-1 py-2 gap-x-1 rounded-md md:h-10 h-8 w-full'
                                        aria-label={isArabic ? "اضـافـة الـي العـربــة" : "Pre Order"}
                                    ...
                                    >
                                        {buyNowLoading == data?.id ?
                                            ...
                                            <>
                                                ...
                                                <div className='text-xs  font-semibold max-md:hidden'>
                                                    {isArabic ? 'انتقل الكرت' : 'Move to Cart'}
                                                </div>
                                            </>
                                        }
                                    </button>
                                ...
                                <button onClick={(e) => {
                                    ...
                                    className={`focus-visible:outline-none bg-[#DC4E4E] fill-[#FFF] border-[#DC4E4E]"border md:h-10 h-8 px-3 rounded-md hover:fill-white hover:bg-[#DC4E4E] hover:border-[#DC4E4E]`} aria-label="add to wishlist">
                                    ...
                                    {ProExtraData[data?.id]?.fbtdata?.fbtlist?.map((fbtpro: any, i: number) => {
                                        ...
                                        return (
                                            <>
                                                ...
                                                <div className={`flex items-center ${props?.devicetype === 'mobile' ? 'gap-1' : ''}`}
                                                    onClick={() => {
                                                        var newfbtdata = fbtProCheck
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center w-full gap-1" key={i}>
                                                        {props?.devicetype !== 'mobile' ?
                                                            <Image
                                                                src={fbtpro?.productdetail?.featured_image ? NewMedia + fbtpro?.productdetail?.featured_image?.image : 'https://partners.tamkeenstores.com.sa/public/assets/new-media/3f4a05b645bdf91af2a0d9598e9526181714129744.png'}
                                                                alt={props.lang === 'ar' ? fbtpro?.productdetail?.name_arabic : fbtpro?.productdetail?.name}
                                                                title={props.lang === 'ar' ? fbtpro?.productdetail?.name_arabic : fbtpro?.productdetail?.name}
                                                                height={50}
                                                                width={50}
                                                                loading='lazy'
                                                            />
                                                        ...
                                                        <div className="w-full">
                                                            ...
                                                            <h4 className="mt-1 text-xs  font-semibold text-dark">
                                                                <>
                                                                    <div className="flex gap-x-1 items-center">
                                                                        {currencySymbol}
                                                                        {fbtprice}
                                                                    </div>
                                                                    <span className="text-xs text-[#DC4E4E] line-through decoration-[#DC4E4E] decoration-2 font-medium">{fbtpro?.productdetail?.price}</span>
                                                                </>
                                                            </h4>
                                                        </div>
                                                    </div>
                        ...
                        {extraData?.freegiftData ?
                            <Transition appear show={isOpen} as={Fragment}>
                                <Dialog as="div" open={isOpen} onClose={() => console.log('test')}>
                                    <Transition.Child
                                        as={Fragment}
                                                ...
                                                <Dialog.Panel as="div" className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8 text-black bg-white">
                                                    <div className="flex bg-white items-center justify-between px-3 py-4 border-b border-[#474B5230]">
                                                        <h5 className='text-sm font-semibold flex items-center gap-x-2 text-[#004B7A] fill-[#004B7A]'>
                                                            ...
                                                            {isArabic ? 'هـدايــا العرض' : 'Select Free Gifts'} {extraData?.freegiftData?.allowed_gifts}
                                                        </h5>
