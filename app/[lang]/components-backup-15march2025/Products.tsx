...
export default function Products(props: any) {
    ...
    // useEffect(() => {
    //     console.log(localStorage.getItem('userWishlist'))
    ...
    // function addToCartButtonsSetting(props: any, data: any) {
    //     if (ProExtraData[data?.id]?.badgeData?.badge_slider || data?.short_description || data?.rating) { }
    // }

    return (
        <>
            {props?.products?.map((data: any, i: React.Key | null | undefined) => {
                if (data?.id) {
                    ...
                    return (
                        <>
                            <div className={`h-auto md:h-auto grid bg-white relative p-2 rounded-lg shadow-md min-w-64 ${props?.className}`} key={i}>
                                <Link
                                ...
                                >
                                    ...
                                    <div className="relative">
                                        {isLoading &&
                                            <div className='animate-pulse'>
                                                <div className='h-[260px] bg-white w-full'></div>
                                            </div>
                                        }
                                        <Image
                                            src={
                                                data?.featured_image
                                                    ? `${Media}${data.featured_image.image}`
                                        ...
                                        <small className="text-xs line-clamp-1 w-36 ltr:text-left rtl:text-right">
                                            {data?.sku || ""}
                                        </small>
                                    </div>
                                    <h2 className="text-primary font-semibold md:text-sm line-clamp-2 text-xs ltr:text-left rtl:text-right">
                                    ...
                                    <div className="text-primary text-xs flex items-center gap-x-2">
                                        ...
                                        {data?.brand?.brand_media_image ? (
                                            <Image
                                                src={
                                                    data?.brand?.brand_media_image
                                                        ? `${NewMedia}${data.brand.brand_media_image.image}`
                                                        : "https://partners.tamkeenstores.com.sa/public/assets/new-media/3f4a05b645bdf91af2a0d9598e9526181714129744.png"
                                                }
                                                alt={
                                                    ProExtraData?.[data?.id]?.badgeData?.badge_slider
                                                        ? props?.lang === "ar"
                                                            ? ProExtraData[data.id].badgeData.badge_slider.title_arabic || ""
                                                            : ProExtraData[data.id].badgeData.badge_slider.title || ""
                                                        : ""
                                                }
                                                title={
                                                    ProExtraData?.[data?.id]?.badgeData?.badge_slider
                                            ...
                                            <p className="text-[#004B7A] text-xs">
                                                {isArabic ? data?.brand?.name_arabic || "" : data?.brand?.name || ""}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div className='md:flex md:items-end md:gap-x-2 mt-2.5 md:mt-1.5 ltr:text-left rtl:text-right'>
                                        {vatOnUs ?
                                            <>
                                                {ProExtraData && Object.keys(ProExtraData).length > 0 && (
                                                    <b className='font-bold text-xl md:text-lg text-[#DC4E4E] leading-3 flex gap-x-1 items-center'>
                                                        {(() => {
                                                            const flashData = ProExtraData[data?.id]?.flash;
                                                            const salePrice = data?.sale_price;
                                                            const price = data?.price;

                                                            if (flashData && salePrice) {
                                                                return flashData.discount_type === 2
                                                                    ? Math.round(salePrice * flashData.discount_amount / 100).toLocaleString('EN-US')
                                                                    : flashData.discount_amount;
                                                            }
                                                            return salePrice ? <>{Math.round((salePrice / 115) * 100).toLocaleString('EN-US')}</> : price.toLocaleString('EN-US');
                                                        })()} {currencySymbol}
                                                    </b>
                                                )}
                                            </>
                                                ...
                                                {ProExtraData && Object.keys(ProExtraData).length > 0 && (
                                                    <b className='font-bold text-xl md:text-lg text-[#DC4E4E] leading-3 flex gap-x-1 items-center'>
                                                        {(() => {
                                                            ...
                                                            return salePrice ? (salePrice - trendyolPrice).toLocaleString('EN-US') : price.toLocaleString('EN-US');
                                                        })()} {currencySymbol}
                                                    </b>
                                                )}
                                            </>
                                    ...
                                    {ProExtraData && Object.keys(ProExtraData).length > 0 && ProExtraData[data?.id]?.freegiftData && (
                                        <div className='flex items-end gap-x-2 mt-2'>
                                            <svg id="fi_3850991" enableBackground="new 0 0 512 512" height="16" viewBox="0 0 512 512" width="16" fill="#219EBC" xmlns="http://www.w3.org/2000/svg">
                                                <g>
                                                    ...
                                                    <path d="m219.67 101.87h72.65c2.68-5.41 4.2-11.5 4.2-17.93 0-22.33-18.2-40.53-40.53-40.53s-40.53 18.2-40.53 40.53c.01 6.44 1.53 12.52 4.21 17.93z"></path>
                                                    <path d="m16.2 148.65v49.55c0 14.89 10.32 27.41 24.17 30.8h187.69v-112.07h-180.15c-17.48 0-31.71 14.23-31.71 31.72z"></path>
                                                    <path d="m40.38 469.91c0 23.21 18.88 42.09 42.09 42.09h145.6v-267.95h-187.69z"></path>
                                                    <path d="m464.09 116.93h-180.13v112.06h187.66c13.86-3.39 24.17-15.91 24.17-30.8v-49.55c.01-17.48-14.22-31.71-31.7-31.71z"></path>
                                    ...
                                    {props?.devicetype === 'mobile' && ProExtraData[data?.id]?.expressdeliveryData && (
                                        <Image
                                            src={props?.lang === 'ar' ? "/icons/express_logo/Express_ar_48.webp" : "/icons/express_logo/Express_en_48.webp"}
                                            width='0' height='0' alt='express_delivery' title='Express Delivery' className='w-full my-3'
                                        />
                                    )}
                                ...
                                <div className={`align__center gap-x-1.5 mt-2.5`}>
                                    {data?.pre_order == 1 ?
                                        <>
                                                        ...
                                                        <div className="flex items-center w-full gap-1" key={i}>
                                                            {props?.devicetype !== 'mobile' ?
                                                                <Image
                                                                    ...
                                                                    alt={isArabic ? fbtpro?.productdetail?.name_arabic : fbtpro?.productdetail?.name}
                                                                    title={isArabic ? fbtpro?.productdetail?.name_arabic : fbtpro?.productdetail?.name}
                                                                    height={50}
                                                                    width={50}
                                                                    loading='lazy'
                                                                />
                                                            ...
                                                            <div className="w-full">
                                                                <h2 className="text-[0.65rem] line-clamp-2">{isArabic ? fbtpro?.productdetail?.name_arabic : fbtpro?.productdetail?.name}</h2>
                                                                    ...
                                                                    <div className="flex w-full items-center justify-between">
                                                                        <div className="flex items-center gap-x-3">
                                                                            <Image
                                                                                ...
                                                                                alt={isArabic ? freegiftdatapro?.productdetail?.name_arabic : freegiftdatapro?.productdetail?.name}
                                                                                title={isArabic ? freegiftdatapro?.productdetail?.name_arabic : freegiftdatapro?.productdetail?.name}
                                                                                height={60}
                                                                                width={60}
                                                                                loading='lazy'
                                                                            ...
                                                                            <div className="flex items-center text-xs">
                                                                                <span>
                                                                                    ...
                                                                                    {extraData?.freegiftData?.discount_type == 2 ?
                                                                                        <p className={`text-[#004B7A] font-bold ${selectedGifts[freegiftdatapro.id] ? 'text-secondary' : ''}`}>
                                                                                            {fgprice != 0 ? currencySymbol + ' ' + fgprice : isArabic ? 'حر' : 'Free'}
                                                                                        </p>
                                                                                        : null}
                                                                                    {extraData?.freegiftData?.discount_type == 3 ?
                                                                                        <p className={`text-[#004B7A] font-bold ${selectedGifts[freegiftdatapro.id] ? 'text-secondary' : ''}`}>
                                                                                            {freegiftdatapro.discount != 0 ? currencySymbol + ' ' + freegiftdatapro.discount : isArabic ? 'حر' : 'Free'}
                                                                                        </p>
                                                                                        : null}
                                            ...
                                            <button className={`focus-visible:outline-none text-white py-1.5 px-4 rounded-md h-10 bg-[#004B7A] border-[#004B7A] border`}
                                                onClick={() => {
                                                    if (Object.keys(selectedGifts).length == extraData?.freegiftData?.allowed_gifts) {
                                                        ...
                                                        topMessageAlartDanger(isArabic ? 'الرجاء اختيار منتجات الهدايا' : 'please select gift products')
                                                    }
                                                }}
                                            >
                                                <div className='text-xs font-semibold'>
