...
import ProductSliderMobile from '../../components/HomePageComponents/ProductSliderMobile';
...
export default function Product({ params, searchParams }: { params: { lang: string, data: any, devicetype: any }, searchParams: any }) {
    ...
    const [PriceAlertProduct, setPriceAlertProduct] = useState(false)
    const [StockAlertProduct, setStockAlertProduct] = useState(false)
    ...
    const [popuppre, setpopuppre] = useState(false)
    const [popupfbt, setpopupfbt] = useState(false)
    ...
    const [expDeliveryText, setexpDeliveryText] = useState('')
    const [expDeliveryQty, setexpDeliveryQty] = useState('')
    const [expDelivery, setexpDelivery] = useState(true)
    const [CheckexpDelivery, setCheckexpDelivery] = useState(true)
    ...
    // CURRENCY SYMBOL //
    const currencySymbol = <svg className="riyal-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1124.14 1256.39" width="22" height="22" style={{ display: 'inline-block', verticalAlign: '-0.125em' }}>
    ...
    //     city.name.toLowerCase().includes(storeSearch.toLowerCase()) ||
    //     city.name_arabic.toLowerCase().includes(storeSearch.toLowerCase()) || 
    ...
    let quantity: any = [];
    for (var i = 0; i < params.data?.data?.quantity; i++) {
    ...
    useEffect(() => {
        ...
        if (params?.data?.data?.mpn) {
            ...
            var language = params?.lang == 'ar' ? 'ar' : 'd2';
            // var language = params?.lang == 'ar' ? params?.data?.data?.flixmedia_ar : params?.data?.data?.flixmedia_en;
        ...
        if (!localStorage.getItem('saveType') && localStorage.getItem('saveType') != data?.savetype) {
            localStorage.setItem('saveType', data?.savetype)
        }
    ...
    return (
        <>
            <MobileHeader
                ...
                lang={params.lang}
                notifications={searchParams?.notifications}
            />
            ...
            <div className="container py-4 max-md:pt-20">
                <div className="md:flex pb-2 pt-10 max-md:pt-0 gap-x-4">
                    ...
                    <div className="flex items-start gap-x-3.5 w-[100%] 2xl:w-[75%]">
                        <div className="h-full mx-auto md:flex justify-center items-center md:bg-white relative md:shadow-md md:rounded-md">
                            <button onClick={() => setImageZoom(true)} className="cursor-zoom-in flex justify-center items-center relative">
                                ...
                                <div className="absolute top-0 w-full h-full">
                                    ...
                                    {data?.custom_badge_en || data?.custom_badge_ar ?
                                        <>
                                            <div className='text-[#EA4335] text-xs absolute ltr:right-0 rtl:left-0 top-0 bg-[#EA433520] md:px-3.5 px-2 py-1 rtl:rounded-tl-lg rtl:rounded-br-lg ltr:rounded-bl-lg ltr:rounded-tr-lg flex gap-x-1 items-center'>
                                                 {isArabic ? "وفر " : "Save"}{" "}{isArabic ? data?.custom_badge_ar : data?.custom_badge_en}{" "}{currencySmallSymbol}
                                            </div>
                            ...
                            <div className="mt-3">
                                ...
                                <h1 className="text-base text-dark  font-semibold py-1.5">{isArabic ? data?.name_arabic : data?.name}</h1>
                    ...
                    <div className="w-full mt-3 md:mt-0">
                        <div className="mt-2 align__center">
                            <h2 className="text-3xl  font-semibold text-[#DC4E4E] flex items-baseline gap-x-1">
                                <div className='flex items-baseline gap-x-1'>
                                    {data?.promotional_price > 0 ?
                                        <>
                                            {getFormattedPrice(Math.round(getDiscountedPrice() - data?.promotional_price))}
                                        </>
                                        :
                                        <>{getFormattedPrice(getDiscountedPrice())}</>
                                    }
                                    {'  '}{currencySymbol}
                                </div>
                                {'  '}
                                {(extraData?.flash || data?.sale_price) && (
                                    <span className="text-lg text-[#5D686F] line-through decoration-[#DC4E4E] decoration-2 decoration-double font-medium">
                                        {getFormattedPrice(getOriginalPrice())}
                                    </span>
                                )}{'  '}
                            </h2>
                            {params.data?.data?.quantity == 0 || params.data?.data?.quantity == null ?
                                <p className="text-[#DC4E4E] text-sm 2xl:text-base">{isArabic ? 'إنتهى من المخزن' : 'Out of Stock'}</p>
                                :
                                <p className="text-primary text-xs">{isArabic ? ' متبقي' : ''} <span className="text-[#219EBC] font-bold">{params.data?.data?.quantity ? params.data?.data?.quantity : '0'} {isArabic ? ' قطع' : ''}</span> {isArabic ? ' فقط في المتجر' : 'Quantity left'}</p>
                            }
                        </div>
                        ...
                        {/* {globalStore?.name ? */}
                            {/* <div className="bg-white p-2 my-2 w-full rounded-md"> */}
                                ...
                                {extraData?.expressdeliveryData && expDelivery ?
                                    <div className='flex items-center gap-4 my-2 bg-[#fde18d] rounded-md p-2'>
                                        <Image
                        ...
                        <div className="align__center gap-2 mt-3">
                            <div className="px-2 pt-2 bg-[#C3E6F170] rounded-md w-1/2 h-32">
                                ...
                                <div className="flex items-center gap-2 justify-center mt-6">
                                    <Link href={`${origin}/${params?.lang}/installment-service-methods`} aria-label="Tabby">
                                        <Image
                                    ...
                                    <Link href={`${origin}/${params?.lang}/installment-service-methods`} aria-label="Tamara">
                                        <Image
                                    ...
                                    {/* <Link href={`${origin}/${params?.lang}/installment-service-methods`} aria-label="Madfu">
                                        <Image
                                    ...
                                    <Link href={`${origin}/${params?.lang}/installment-service-methods`} aria-label="misspay">
                                        <Image
                            ...
                            <div className="px-2 pt-2 bg-[#C3E6F170] rounded-md w-1/2 h-32 relative">
                                ...
                                <Link href={`${origin}/${params?.lang}/installment-service-methods`} aria-label="baseeta" className="flex justify-center mt-6">
                                    <Image
                ...
                <div className="md:grid md:grid-cols-3 md:gap-x-4 py-2">
                    ...
                    <div className="w-full">
                        {extraData?.freegiftdata?.freegiftlist?.slice(0, 1).map((freegiftdatapro: any, i: number) => {
                            return (
                                <button onClick={openModal} className="focus-visible:outline-none bg-[#EEF8FC] shadow-md rounded-md p-3 w-full max-md:my-3">
                                    <p className="text-[#004B7A] fill-[#004B7A] text-sm font-bold flex items-center gap-x-1.5 mb-3">
                                        ...
                                        {extraData?.freegiftdata?.freegiftlist.length === extraData?.freegiftdata?.allowed_gifts ?
                                            ...
                                            extraData?.freegiftdata?.discount_type === 1 ?
                                                <>{isArabic ? `هديا (${extraData?.freegiftdata?.allowed_gifts}) مجانية مضافة` : `Choose (${extraData?.freegiftdata?.allowed_gifts}) Free Gift`}</>
                                                :
                ...
                {/* {globalStore?.name ? 
                    <div className="bg-white p-2 my-2 w-full rounded-md">
                        <div className='flex gap-2 justify-start items-center'>
                            ...
                            <h6 className='font-bold text-base'>{params?.lang == "ar" ? "احصل عليه بطريقتك!" : "Get it your way!"}</h6>
                        </div>
                        <div className='bg-[#fde18d] rounded-md p-2 w-full text-sm space-y-2'>
                            <div className='flex gap-3 justify-start items-center'>
                                <p>{isArabic ? 'الاستلام من المتجر' : 'Store pick-up in'} <button className="text-[#219EBC] font-semibold" 
                                onClick={() => {
                                    setIsOpenModal(true)
                                }}>{isArabic ? globalStore?.showroom_data?.name_arabic : globalStore?.showroom_data?.name}</button></p>
                            ...
                            <div className='flex gap-3 justify-start items-center'>
                                ...
                                <p>{isArabic ? 'ستكون المجموعة متاحة خلال الساعة القادمة' : 'Collection will be available in the next 1 hour(s)'}</p>
                            </div>
                        </div>
                    </div>
                :null} */}
                        ...
                        {data?.features?.slice(0, !keyFeature ? 3 : 5).map((data: any) => (
                            <li className="flex items-center gap-x-2 mb-2 ltr:ml-3 rtl:mr-3" key={data?.id}>
                                <Image
                                    ...
                                    loading='lazy'
                                />
                                <p className="text-xs font-normal">{isArabic ? data?.feature_ar : data?.feature_en}</p>
                            </li>
                        ))}
                    </ul>
                ...
                {upsaleproductdata ?
                    <div className='my-6'>
                        {/* <h3 className='text-base  font-semibold'>{isArabic ? 'المنتجات الموصى بها' : 'Recommended Products'}</h3> */}
                        <div className='mt-2 pb-2'>
                            <ProductSliderMobile title={isArabic ? 'المنتجات الموصى بها' : 'Recommended Products'} titleSlider={isArabic ? 'المنتجات الموصى بها' : 'Recommended Products'} lang={params.lang == 'ar' ? true : false} dict={dict.products} products={upsaleproductdata} devicetype={params?.devicetype} headingRequired={true} />
                        </div>
                    </div>
                    : null}
            </div>
            ...
            {data?.description && data?.description_arabic ?
                <div className="container mb-6">
                    <h3 className='text-base font-semibold mb-1'>{params.lang === 'ar' ? 'معلومات إضافية' : 'More Details'}</h3>
                    <div className={`relative bg-white ${descriptionMore == true ? "max-h-full" : 'h-[400px] overflow-hidden'}`}>
            ...
            {data?.specs?.length >= 2 ?
                <>
                    <div className="container">
                        <h3 className='text-base  font-semibold'>{params?.lang == 'ar' ? 'تفاصيل المنتج الحالي' : 'Specifications'}</h3>
                        {/* <Link href="#" className='text-[#219EBC] hover:underline'>المواصفات</Link> */}
                    </div>
                            ...
                            return (
                                <div className="py-3 border-2 border-[#5D686F30] border-t-0 border-l-0 border-r-0">
                                    <div className="grid md:grid-cols-8 container">
                                        <label className="md:py-4 font-semibold text-sm md:text-xs max-md:mb-4">{params?.lang == 'ar' ? item?.heading_ar : item?.heading_en}</label>
                                    ...
                                    {data?.reviews?.map((review: any, i: number) => (
                                        <div className="" key={i}>
                                            ...
                                            <div className="text-sm mt-5">
                                                ...
                                                <p className="font-normal text-xs text-[#5D686F]">{review?.review}</p>
                                            </div>
                                            {i + 1 === review?.length ?
                                                null
                                                :
            ...
            {productdata ?
                <div className='my-6'>
                    ...
                    <div className='mt-2 pb-2'>
                        {productdata ?
                            <ProductSlider lang={params.lang} dict={dict.products} products={productdata} />
                            : null}
                    </div>
                </div>
                : null}
            ...
            {/* <div className='my-6 container'>
                ...
                <div className='mt-2 pb-2'>
                    <ProductSlider lang={params.lang} dict={dict.products} products={highestviewprodata} />
                </div>
            </div> */}
            ...
            <div className="fixed bottom-0 w-full p-3 bg-white shadow-md border-t border-[#5D686F26] z-10">
                ...
                <div className="flex items-center justify-between gap-x-1 md:gap-x-3">
                        ...
                        <>
                                ...
                                <>
                                    {StockAlertProduct ?
                                        <button onClick={(e: any) => { handleStockAlert() }} className="focus-visible:outline-none btn border border-[#DC4E4E] bg-[#DC4E4E] text-white p-3 rounded-md w-full font-medium">
                                            {isArabic ? 'شراء الان' : 'Remove Stock Alert'}
                                        </button>
                                        :
                                        <button onClick={(e: any) => { handleStockAlert() }} className="focus-visible:outline-none btn border border-[#004B7A] bg-[#004B7A] text-white p-3 rounded-md w-full font-medium">
                                            {isArabic ? 'شراء الان' : 'Setup Stock Alert'}
                                        </button>
                                    }
                                </>
                                :
                            ...
                            {quantity == 0 || quantity == null ?
                                ...
                                <button
                                    ...
                                    className="border focus-visible:outline-none rounded-md py-2.5 px-2 h-auto w-36 border-[#004B7A] text-[#004B7A] fill-[#004B7A]  font-semibold text-base bg-[#5D686F05] flex items-center justify-between"
                                >
                                    {sqty}
                                    <div className="flex items-center gap-x-1.5">
                                    ...
                                    <Dialog.Panel as="div" className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8 text-black bg-white">
                                                                                ...
                                                                                <span>
                                                                                    ...
                                                                                    {extraData?.freegiftdata?.discount_type == 2 ?
                                                                                        <p className={`text-[#004B7A] font-bold ${selectedGifts[freegiftdatapro.id] ? 'text-secondary' : ''}`}>
                                                                                            ...
                                                                                            {fgprice != 0 ? (currencySymbol + ' ' + fgprice) : params.lang === 'ar' ? 'حر' : 'Free'}
                                                                                        </p>
                                                                                        : null}
                                                                                    {extraData?.freegiftdata?.discount_type == 3 ?
                                                                                        <p className={`text-[#004B7A] font-bold ${selectedGifts[freegiftdatapro.id] ? 'text-secondary' : ''}`}>
                                                                                            {freegiftdatapro?.discount != 0 ?
                                                                                                (currencySymbol + ' ' + freegiftdatapro?.discount) : params.lang === 'ar' ? 'حر' : 'Free'}
                                                                                        </p>
                                                                                        : null}
                                        ...
                                        {extraData?.freegiftdata?.freegiftlist?.filter((e: any) => e?.discount > 0)?.length > 0 ?
                                            <div className="border-t border-[#5D686F30] py-3 px-3 gap-2 flex items-center justify-end">
                                                ...
                                                >
                                                    <div className='text-xs font-semibold'>
                                                        {isArabic ? "يتخطى" : "Select This"}{' '}{extraData?.freegiftdata?.allowed_gifts}{' '}{isArabic ? "يتخطى" : "Gift"}
                                                    </div>
                                                </button>
                                            </div>
                                            : null}
                            ...
                            >
                                <Dialog.Panel as="div" className="panel border-0 p-0 rounded-lg overflow-hidden w-full h-auto max-w-5xl my-8 text-black bg-white relative">
                                    ...
                                    <div className="border-t border-[#5D686F26] p-3 flex gap-3">
                                        ...
                                        {data?.gallery?.slice(0, !showGallery ? 4 : 400).map((item: any, i: any) => {
                                            return (
