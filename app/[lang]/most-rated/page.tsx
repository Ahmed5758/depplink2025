...
export default function Category({ params, searchParams }: { params: { lang: string, slug: string, data: any }, searchParams: any }) {
  ...
  const filter = () => {
    ...
    if (Object.keys(filterdata).length >= 1) {
      router.push(`/${params.lang}/most-rated${result}`, { scroll: false })
      router.refresh();
    }
  }
            ...
            {CatData?.productData?.products?.data?.length ?
              ...
              <div className="container my-10 flex items-center justify-center py-8">
                <div className='text-center'>
                  ...
                  <button className="btn focus-visible:outline-none bg-[#004B7A] w-72 p-2.5 rounded-md text-sm 2xl:text-base text-white mt-6" onClick={() => router.push('/')}>{params.lang == 'ar' ? 'تسوق المنتجات' : 'Shop products'}</button>
                </div>
              </div>
            }
          </div>
