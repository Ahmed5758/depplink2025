...
export default function ContactUs({ params }: { params: { lang: string } }) {
    ...
    return (
        <>
            <MobileHeader type="Third" lang={params.lang} pageTitle={params.lang == 'ar' ? 'تواصل معنا' : 'Contact Us'} />
            <div className="container py-16 md:py-4">
                <div className="mb-6 md:my-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="">
                            <h1 className=" font-semibold text-xl hidden md:block">{params.lang == 'ar' ? 'تـواصـل مـعـنــا' : 'Contact Us'}</h1>
                            <p className='text-[#5D686F] text-xs mb-3'>{params.lang == 'ar' ? 'يرجي ادخال واكمال البيانات التالية والضغط علي ارسال وسيتم التواصل معك في اقرب وقت' : 'Please enter and complete the following information and click submit and we will contact you as soon as possible'}</p>
                            <div className='mb-2'>
                                <label className="text-sm font-medium text-[#344054]">{params.lang == 'ar' ? 'الاسم بالكامل' : 'Full Name'}</label>
                                <div className={`border border-[#${fullNameError ? 'dc4e4e' : '004B7A'}] focus-visible:outline-none hover:border-primary bg-white rounded p-3`}>
                                    <div className="flex items-center gap-3 fill-[#004B7A]">
                                        ...
                                        <input id="iconLeft" type="text" placeholder={params.lang == 'ar' ? 'الاسم بالكامل' : 'Full Name'} className="text-sm focus-visible:outline-none w-full"
                                            onChange={(e: any) => {
                                ...
                                {fullNameError ? (
                                    ...
                                    >
                                        {params.lang == 'ar' ? 'يرجى إضافة الاسم' : 'Please fill name'}
                                    </label>
                                ) : null}
                            </div>
                            <div className='mb-2'>
                                <label className="text-sm font-medium text-[#344054]">{params.lang == 'ar' ? 'بريد إلكتروني' : 'E-mail'}</label>
                                <div className={`border border-[#${emailError ? 'dc4e4e' : '004B7A'}] focus-visible:outline-none hover:border-primary bg-white rounded p-3`}>
                                    <div className="flex items-center gap-3 fill-[#004B7A]">
                                        ...
                                        <input id="iconLeft" type="mail" placeholder={params.lang == 'ar' ? 'بريد إلكتروني' : 'E-mail'} className="text-sm focus-visible:outline-none w-full"
                                ...
                                {emailError ? (
                                    <label
                                        ...
                                        className="text-sm font-medium text-[#dc4e4e]"
                                    >
                                        {params.lang == 'ar' ? 'يرجى إضافة البريد الإلكتروني' : 'Please fill email'}
                                    </label>
                                ) : null}
                                <label
                                    htmlFor="invalid"
                            ...
                            <div className='mb-2'>
                                <label className="text-sm font-medium text-[#344054]">{params.lang == 'ar' ? 'رقم التليفون' : 'Phone Number'}</label>
                                ...
                                {phoneNumberError ? (
                                    <label htmlFor="invalid" className="text-sm font-medium text-[#dc4e4e]">
                                        {params.lang == 'ar' ? 'يرجى إضافة رقم الجوال' : 'Please fill phone number'}
                                    </label>
                                ) : null}
                            </div>
                            <div className='mb-2'>
                                <label className="text-sm font-medium text-[#344054]">{params.lang == 'ar' ? 'نوع الشكوى' : 'Subject'}</label>
                                <div className={`border border-[#004B7A] focus-visible:outline-none hover:border-primary bg-white rounded p-3`}>
                                    <div className="flex items-center gap-3 fill-[#004B7A]">
                                        <input id="iconLeft" type="text" placeholder={params.lang == 'ar' ? 'نوع الشكوى' : 'Subject'} className="text-sm focus-visible:outline-none w-full"
                                            onChange={(e: any) => {
                            ...
                            <div className='mb-2'>
                                <label className="text-sm font-medium text-[#344054]">{params.lang == 'ar' ? 'تفاصيل الشكوى' : 'Details'}</label>
                                <div className={`border border-[#004B7A] focus-visible:outline-none hover:border-primary bg-white rounded p-3`}>
                                    <div className="flex items-center gap-3 fill-[#004B7A]">
                                        <input id="iconLeft" type="text" placeholder={params.lang == 'ar' ? 'تفاصيل الشكوى' : 'Details'} className="text-sm focus-visible:outline-none w-full"
                                            onChange={(e: any) => {
                            ...
                            <div className='mb-2'>
                                <label className="text-sm font-medium text-[#344054]">{params.lang == 'ar' ? 'رقم الهاتف' : 'Note'}</label>
                                <div className={`border border-[#${notesError ? 'dc4e4e' : '004B7A'}] focus-visible:outline-none hover:border-primary bg-white rounded p-3`}>
                                    <div className="flex items-start gap-3 fill-[#004B7A]">
                                        ...
                                        <textarea placeholder={params.lang == 'ar' ? 'رقم الهاتف' : 'Note'} rows={3} wrap="soft" className="focus-visible:outline-none text-sm w-full" value={notes} id="message2" onChange={(e: any) => setnotes(e.target.value)}></textarea>
                                    </div>
                                </div>
                                {notesError ? (
                                    <label
                                        ...
                                        className="text-sm font-medium text-[#dc4e4e]"
                                    >
                                        {params.lang == 'ar' ? 'يرجى إضافة الملاحظات' : 'Please fill notes'}
                                    </label>
                                ) : null}
                            </div>
                        </div>
                        <Image alt={params.lang == 'ar' ? 'تـواصـل مـعـنــا' : 'Contact Us'} title={params.lang == 'ar' ? 'تـواصـل مـعـنــا' : 'Contact Us'} src="/images/ContactUs.jpeg" width={0} height={0} className="w-full h-auto hidden md:block" />
                    </div>
                    <h2 className="text-base mb-2 font-semibold mt-8">{params.lang == 'ar' ? 'بيـانـات التواصــل ومــعلومـــات الموقع' : 'Contact Information'}</h2>
                    <div className="bg-white p-3 boder rounded-md shadow-md">
                        <div className="flex items-center gap-2 mb-2">
                            <label className="font-semibold text-xs md:w-1/12">{params.lang == 'ar' ? 'البريد الالكتروني:' : 'E-mail:'}</label>
