...
export default async function OrderDetailsLayout({ children }: { children: React.ReactNode }) {
    ...
    const jsonLd = [
        {
            ...
            itemListElement: [
                ...
                },
                {
                    "@type": "ListItem",
                    position: 2,
                    name: lang === 'ar' ? 'تفاصيل الطلب' : 'Order Details',
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Order Details"
            : footer?.data?.meta_title_ar ?? "تفاصيل الطلب | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
              "Tamkeen Stores Order Details"
            : footer?.data?.meta_description_ar ??
              "تفاصيل الطلب معارض تمكين";

    // In /[lang]/orderdetails/[...slug], slugParts are ONLY the [...slug] bits
