...
export default async function OrderListingLayout({ children }: { children: React.ReactNode }) {
    ...
    const jsonLd = [
        {
            ...
            itemListElement: [
                ...
                },
                {
                    "@type": "ListItem",
                    position: 2,
                    name: lang === 'ar' ? 'قائمة الطلبات' : 'Order Listing',
...
export async function generateMetadata(): Promise<Metadata | null> {
    ...
    const metaTitle =
        lang === "en"
            ? footer?.data?.meta_title_en ?? "Tamkeen Stores Order Listing"
            : footer?.data?.meta_title_ar ?? "قائمة الطلبات | معارض تمكين";

    const metaDescription =
        lang === "en"
            ? footer?.data?.meta_description_en ??
            "Tamkeen Stores Order Listing"
            : footer?.data?.meta_description_ar ??
            "قائمة الطلبات معارض تمكين";

    // In /[lang]/orderlisting/[...slug], slugParts are ONLY the [...slug] bits (not "orderlisting")
