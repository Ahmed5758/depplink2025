    ...
    return (
        <>
            {comapareData?.user?.compares ?
                <div className="container py-4">
                    <div className="py-6">
                        <div className="flex items-center justify-between">
                            <h1 className=" font-semibold text-lg 2xl:text-xl">{params.lang == 'ar' ? 'قائمة مقارنة المنتجات' : 'Product Comperission'} ({comapareData?.user?.compares?.length}/4)</h1>
                            <button className="text-xs focus-visible:outline-none font-semibold underline text-[#EA4335]" onClick={(e: any) => { DeleteAllCompare() }}>{params.lang == 'ar' ? 'حـذـف كــل الــمنتــجات' : 'Delete all products'}</button>
                        </div>
                        ...
                        <div className="my-10">
                            ...
                            <div className="grid grid-cols-5 text-sm text-center  font-semibold">
                                <div className="bg-[#B1553340] border border-[#5D686F30] p-4 flex items-center justify-center text-[#B15533]">{params.lang == "ar" ? 'خصـائص ومـميزات المنتج' : 'Product Specifications'}</div>
                                {comapareData?.user?.compares?.map((data: any, i: React.Key | null | undefined) => {
                                    return (
                                    ...
                                    [...Array(4 - comapareData?.user?.compares?.length)].map((data: any, i: React.Key | null | undefined) => {
                                        return (
                                            <div className="bg-[#5D686F20] border border-[#5D686F30] p-4 flex items-center justify-center text-[#00002C] fill-[#00002C] gap-x-3">
                                                ...
                                                {params.lang == "ar" ? 'إضافة مــنــتج اخــر الي الـمقـارنة' : 'Product Specifications'}
                                            </div>
                                        )
                                    })
                                    :
                            ...
                            <div className="grid grid-cols-5 text-sm text-center  font-semibold bg-white">
                                ...
                                <div className="border border-[#5D686F30] p-4 flex items-center justify-center text-[#B15533]">{params.lang == "ar" ? 'الأسعار' : `Gifts`}</div>
                                            ...
                                            {ProExtraData[data?.product_id]?.freegiftData ?
                                                <div className='flex items-end gap-x-2 mt-2'>
                                                    ...
                                                    <div className='text-xs font-regular'>
                                                        {params.lang == 'ar' ? "يوجد هدية مع الـمنتج" : "With Free Gift"}
                                                    </div>
                                                </div>
                                                :
                            ...
                            <div className="grid grid-cols-5 text-sm text-center  font-semibold bg-white">
                                <div className="border border-[#5D686F30] p-4 flex items-center justify-center text-[#B15533]">{params.lang == "ar" ? 'رقم الموديل' : `Model Number`}</div>
                            ...
                            <div className="grid grid-cols-5 text-sm text-center  font-semibold bg-white">
                                <div className="border border-[#5D686F30] p-4 flex items-center justify-center text-[#B15533]">{params.lang == "ar" ? 'الأسعار' : `Price's`}</div>
                                {comapareData?.user?.compares?.map((data: any, t: React.Key | null | undefined) => {
                                    return (
                                        <div key={t} className="border border-[#5D686F30] p-4 flex items-center justify-center text-[#004B7A] gap-x-2 font-bold text-lg">
                                            <div className="flex items-center gap-x-1">
                                            {currencySymbol}{Intl.NumberFormat('en-US').format(data?.product?.sale_price)}
                                            </div>{' '}<span className="text-[#EB5757] decoration-[#DC4E4E] decoration-2 line-through  font-semibold text-sm">{Intl.NumberFormat('en-US').format(data?.product?.price)}{' '}{params.lang == "ar" ? 'ر.س' : 'SR'}</span>
                                        </div>
                                    )
                                })
                                }
                                    ...
                                    return (
                                        <div className="p-4" key={b}>
                                            <button onClick={(e) => {
                                                ...
                                                className="focus-visible:outline-none btn text-sm bg-[#004B7A] p-2.5 rounded-md text-white w-full">{params.lang == "ar" ? 'اضف الي العربة' : 'Add to Cart'}</button>
                                            <button onClick={(e) => {
                                                removecompareProduct(data.product_id,true)
                                            }}
                                            className="underline font-semibold text-sm text-[#DC4E4E] mx-auto w-full mt-2">{params?.lang === 'ar' ? 'إزالة من المقارنة' : 'Remove From Compare'}</button>
                                        </div>
                                    )
                                })
                                }
                                                ...
                                                <Dialog.Panel as="div" className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8 text-black bg-white">
                                                    <div className="flex bg-white items-center justify-between px-5 pt-4">
                                                        <h5 className='text-base font-[600] flex items-center gap-x-2 text-[#004B7A] fill-[#004B7A]'>
                                                            ...
                                                            {params.lang == 'ar' ? 'هـدايــا العرض' : 'Select Free Gifts'} {extraData?.freegiftData?.allowed_gifts}
                                                        </h5>
                                                                                            ...
                                                                                            <span>
                                                                                                {extraData?.freegiftData?.discount_type == 1 ?
                                                                                                    <p className={`text-[#004B7A] font-bold ${selectedGifts[freegiftdatapro.id] ? 'text-secondary' : ''}`}>
                                                                                                        {params.lang == 'ar' ? 'هدية مجانية' : 'Free Gift Item'}
                                                                                                    </p>
                                                                                                    : null}
                                                                                                ...
                                                                                                <h6 className={` font-semibold text-[#5D686F] mt-3.5 text-left rtl:text-right ${selectedGifts[freegiftdatapro.id] ? 'text-secondary' : ''}`}>
                                                                                                    {params.lang == 'ar' ? 'العلامة' : 'Brand'} {params.lang == 'ar' ? freegiftdatapro?.productdetail?.brand?.name_arabic : freegiftdatapro?.productdetail?.brand?.name}</h6>
                                                                                            </span>
                                                                                        </div>
                                                                                    </div>
                                                                                </div>
                ...
                <div className="container py-4">
                    <div className="py-6">
                        <h1 className="text-center  font-semibold text-lg 2xl:text-xl">{params.lang == 'ar' ? 'لا يوجد منتج!' : 'There is no product!'}</h1>
                    </div>
                </div>
            }
        </>
