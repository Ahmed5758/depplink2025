...
import { setDiscountRule, setDiscountRuleBogo } from '../cartstorage/cart';
...
export default function Login({ params, searchParams }: {
    params: {
        dict: any; lang: string, slug: string, data: any
    }, searchParams: any
}) {
    ...
    const { setglobalCity } = useContext(GlobalContext);
    ...
    //       return false;
    //     }
    //     const token:any = await executeRecaptcha('submit'); // Use a specific action name
    //     setToken(token);
    //     return token
    // };
    ...
    const changeLang = (lang: string, checkOtp: boolean) => {
        if (searchParams?.type && lang) {
            var url = '/' + lang + path.split(`/${params?.lang}`)[1] + '?type=' + searchParams?.type
        }
        else if (lang) {
            var url = '/' + lang + path.split(`/${params?.lang}`)[1]
        }
        else {
            var url = path.replace(`${origin}/${params?.lang}`, `/${lang}`)
        }
        router.push(url);
        router.refresh()
    };
    ...
    const CheckPhoneNumber = async() => {
        // var token = await handleCaptcha()
        // console.log(token)
        // return false
        // event.preventDefault();
        const recaptchaValue = recaptchaRef.current.getValue();
        // console.log(recaptchaValue); // Send this to your server for verification
        // setLoginBtnLoading(false)
        // return false
        if(!recaptchaValue){
            setLoginBtnLoading(false)
        ...
        // const secretKey = '6Ld5cE0qAAAAAHQ3J_hev7nvhAtq-bcbAMxL0haP'; // Replace with your secret key
        ...
        // const datacaptcha:any = await response.json();
        ...
        // if (datacaptcha.success) {
            ...
            post('user-login', data).then((responseJson: any) => {
                ...
                if (responseJson?.success === true) {
                    ...
                    // router.push(`/${params.lang}/signup`)
                    if (searchParams?.type) {
                        router.push(`/${params.lang}/signup?type=${searchParams?.type}`)
                    }
                    else{
                        router.push(`/${params.lang}/signup`)
                    }
        ...
        //     return false;
        //   }
        // const tokenData = await executeRecaptcha('submit'); // Use a specific action name
        // // setToken(token);
        // const response = await fetch(`https://www.google.com/recaptcha/api/siteverify`, {
        //     method: 'POST',
        //     headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        //     body: new URLSearchParams({
        //       secret: '6LfGjE0qAAAAAAOlPbyKjEoND0Hd8y1EwG-Pq9sf',
        //       response: tokenData,
        //     }),
        //   });
        ...
        //   if (datacaptcha.success && datacaptcha.score >= 0.5) {
            ...
            // return new Response(JSON.stringify({ message: 'CAPTCHA verified successfully!' }), { status: 200 });
        //   } else {
        //     setLoginBtnLoading(false)
        //     setcaptchaErrorStatus(true)
        //     return false
        ...
        post('check-otp', dataotp).then((responseJson: any) => {
            if (responseJson?.success === true) {
                ...
                get(`user-profile/${localStorage.getItem("userid")}`).then((responseJson: any) => {
                    ...
                    // setCompareCount(localStorage.getItem('compareCount'))
                    // setWishlistCount(localStorage.getItem('wishlistCount'))
                    // setOrderCount(localStorage.getItem('orderCount'))
                    // setUserid(localStorage.getItem("userid"))
                    // Global State Update
                        ...
                        <div>
                            ...
                            {otpbox == true ?
                                <>
                                    ...
                                    <p className="text-xs font-normal mt-1 text-[#5D686F] text-center">{params.lang == 'ar' ? 'تم إرسال OTP إلى' : 'OTP has been sent to'} <br /><span className="text-[#004B7A]">+966{phoneNumber}</span> <br /> {params.lang == 'ar' ? 'عبر الرسائل القصيرة' : 'via SMS'}</p>
                                    ...
                                    <div className="text-center">
                                        {minutes == 0 && seconds == 0 ?
                                            ...
                                            :
                                            <p className="text-xs font-normal text-center text-[#5D686F]">{params.lang === 'ar' ? 'The code is valid for ' + minutes + ': ' + seconds + ' Mins' : 'The code is valid for ' + minutes + ':' + seconds + ' Mins'}</p>
                                        }
                                ...
                                <>
                                    {/* LOGIN BOX */}
                                    <h1 className="text-xl font-bold">{params.lang == 'ar' ? 'لديك حساب؟ سجل الدخول' : 'Already a member? Sign in!'}</h1>
                                    <p className="text-xs font-normal mt-1 text-[#5D686F]">{params.lang == 'ar' ? 'قم بتسجيل الدخول حتى تتبع طلباتك وتصل بسهولة إلى قائمة مفضلاتك.' : 'Get access to your orders, wishlist and recommendations to products you are interested in!'}</p>
                                    ...
                                    {captchaErrorStatus ?
                                        <p className='font-semibold text-sm'>
                                            {params.lang === 'ar' ? 'يرجى تحديد المربع للتحقق من أنك إنسان.' : 'Please check the box to verify that you are human.'}
                                        </p>
                                        : null}
                                    {loginErrorStatus ?
                                        <p>
                                            {params.lang === 'ar' ? 'لم يتم العثور على هذا الحساب' : 'This account has not found!'}
                                        </p>
                                        : null}
                            ...
                            <h2 className="text-xs text-center mt-1">{params.lang == 'ar' ? `سجل الآن في حال عدم وجود حساب لديك` : `If you don't have a account please click on`}{' '}
                                <button className="focus-visible:outline-none font-bold underline" onClick={() => {
                                    router.push(`/${params.lang}/signup`)
                                }}>{params.lang == 'ar' ? 'تسجيل الآن' : 'Sign Up'}</button>
                            </h2>
                        </div>
                        <div className="w-full md:w-80 md:ml-auto mt-10 md:mt-0">
                            <div className="bg-white rounded-md shadow-md">
                                <div>
                                    <Link href={`/${params?.lang}/${notificationsListing?.link}`} className="focus-visible:outline-none w-full text-left">
                                        {notificationsListing?.image ?
                                            <Image loading='lazy' src={notificationsListing?.image} alt={params.lang === 'ar' ? notificationsListing?.title_arabic : notificationsListing?.title} title={params.lang === 'ar' ? notificationsListing?.title_arabic : notificationsListing?.title} height={0} width={0} className="h-auto w-full rounded-tl-md rounded-tr-md"  sizes="(max-width: 640px) 100vw, (max-width: 768px) 100vw, (max-width: 1024px) 100vw, 100vw" />
                                            :
                                ...
                                <div className="px-5 pb-5">
                                    <hr className="my-4 opacity-10" />
                                    <div className="mt-4">
                                        <h3 className="font-semibold text-sm">{params.lang === 'ar' ? 'هل انت جديد في معارض تمكين؟ سجل الآن' : 'New to Tamkeen Stores? Register!'}</h3>
                                        <div className="mt-4">
                                        ...
                                        <button className="focus-visible:outline-none btn border border-[#004B7A] hover:bg-[#004B7A] text-[#004B7A] p-3 rounded-full w-full hover:text-white fill-white font-semibold text-sm mt-4"
                                            onClick={() => { router.push(`/${params.lang}/signup`) }}>
                                            {params.lang == 'ar' ? 'سجل الآن' : 'Register'}
                                        </button>
                                    </div>
                                </div>
                            </div>
